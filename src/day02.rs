...
pub fn day02(input_type: InputType, manual_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for row in &data {
        ...
        for i in (2..row.len()).step_by(2) {
            ...
            let colour_cube = &row[i+1];
            // println!("{} {} {}", i, row[i], colour_cube);
            // assign colours
            if colour_cube.contains("red") {game_cubes.red = num_cube}
            if colour_cube.contains("blue") {game_cubes.blue = num_cube}
            if colour_cube.contains("green") {game_cubes.green = num_cube}
            if colour_cube.contains(";") | (i == row.len() - 2){
                // println!("tally up");
                // println!("red {}, green {}, blue {}", game_cubes.red, game_cubes.green, game_cubes.blue);
                // end of round, tally up
                if (game_cubes.red > max_cubes.red) | (game_cubes.blue > max_cubes.blue) |
                    (game_cubes.green > max_cubes.green) | (game_cubes.red + game_cubes.blue +game_cubes.green > max_val)
                {
